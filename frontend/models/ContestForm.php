...
class ContestForm extends Model
{
    ...
    public function upload()
    {
        if ($this->validate()) {
            ...
            $contest->contest_image = '/uploads/'.Yii::$app->user->id.'-'.$this->file_image->baseName . '.'. $this->file_image->extension;


           // $this->file_image = UploadedFile::getInstance($contest, 'file_image');

            $this->file_image->saveAs(Yii::getAlias('@frontend').'/web'.$contest->contest_image);
            $contest->save();
            $this->_id = $contest->id;
           // if ($contest->save()) {
            //    return true;
